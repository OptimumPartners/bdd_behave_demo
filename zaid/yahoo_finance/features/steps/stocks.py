...
def step_impl(context):
    ...
    for stock in _result:
        ...
        if type(stock["symbol"]) is None:
            err.append("stock in index " + str(counter) + " dose not have symbol")
        if type(stock["price"]) is None:
            err.append("stock in index " + str(counter) + " dose not have price")
        counter += 1
        if err != []:
            errors.append(err)
    if errors != []:
...
def step_impl(context):
    ...
    for stock in _result:
        ...
        browser = (
            Common.firefox()
        )  # replace firefox with chrome if u want to run it on chrome
        browser.get(YAHOO_URL)
        browser.find_element(By.CSS_SELECTOR, SEARCH_INPUT_locator).send_keys(
            stock["symbol"]
        )
        ...
        if round(float(stock["price"]), 2) not in arr:
            err.append(
                [
                    "The two prices is not equal for " + stock["symbol"],
                    arr,
                    round(float(stock["price"]), 2),
                ]
            )
