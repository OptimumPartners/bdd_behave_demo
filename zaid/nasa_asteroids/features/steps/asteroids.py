...
def step_impl(context, method):
    global _method
    if method == "":
        assert False, "endpoint method is not provided"
    _method = method
    ...
    for asteroid in _asteroids:
        ...
        if type(asteroid["absolute_magnitude_h"]) != float:
            err.append(
                "asteroid in index "
                + str(counter)
                + " dose not have absolute_magnitude_h"
            )

        minDiameterInkilometers = asteroid["estimated_diameter"]["kilometers"][
            "estimated_diameter_min"
        ]
        maxDiameterInkilometers = asteroid["estimated_diameter"]["kilometers"][
        ...
        if minDiameterInkilometers > maxDiameterInkilometers:
            err.append(
                " asteroid in index "
                + str(counter)
                + " dose not have uncorect diameter"
            )

        diameterFromKMetarsToMeters = round(
            asteroid["estimated_diameter"]["kilometers"]["estimated_diameter_min"]
            * 1000,
            5,
        )
        diameterInMeters = round(
            asteroid["estimated_diameter"]["meters"]["estimated_diameter_min"], 5
        )
        if diameterFromKMetarsToMeters != diameterInMeters:
            err.append(
                "asteroid in index "
                + str(counter)
                + " dose not have the same distance in m and km"
            )
        ...
        if type(asteroid["is_potentially_hazardous_asteroid"]) != bool:
            err.append(
                "asteroid in index "
                + str(counter)
                + ' dose not have "is_potentially_hazardous_asteroid" proparty'
            )
...
def step_impl(context):
    ...
    for asteroid in _asteroids:
        ...
        err = []
        browser = Common.firefox() #to run it on chrome replace firfox with chrome
        browser.get(asteroid["nasa_jpl_url"].replace("http://", "https://"))

        absolute_magnitude_h = browser.find_element(By.XPATH, ABSOLUTE_SELECTOR).text
        if str(asteroid["absolute_magnitude_h"]) != str(absolute_magnitude_h):
            err.append(
                [
                    "asteroid in the index of  "
                    + str(counter)
                    + "  in the api array dose not have the same absolute_magnitude_h in UI",
                    str(absolute_magnitude_h),
                    str(asteroid["absolute_magnitude_h"]),
                ]
            )
        ...
        if str(SpkId) != str(asteroid["id"]):
            err.append(
                [
                    "asteroid in the index of  "
                    + str(counter)
                    + "  in the api array dose not have the same id in UI",
                    SpkId,
                    str(asteroid["id"]),
                ]
            )

        eccentricity = browser.find_element(By.XPATH, ECCENTRICITY_SELECTOR).text
        if str(eccentricity) != str(asteroidFullData["orbital_data"]["eccentricity"]):
            err.append(
                [
                    "asteroid in the index of  "
                    + str(counter)
                    + "  in the api array dose not have the same eccentricity in UI",
                    str(eccentricity),
                    str(asteroidFullData["orbital_data"]["eccentricity"]),
                ]
            )

        semiMajorAxis = browser.find_element(By.XPATH, MAJOR_AXIS_SELECTOR).text
        if str(semiMajorAxis) != str(
            asteroidFullData["orbital_data"]["semi_major_axis"]
        ):
            err.append(
                [
                    "asteroid in the index of  "
                    + str(counter)
                    + "  in the api array dose not have the same semi_major_axis in UI",
                    str(semiMajorAxis),
                    str(asteroidFullData["orbital_data"]["semi_major_axis"]),
                ]
            )

        perihelionDistance = browser.find_element(
            By.XPATH, PERIHELION_DISTANCE_SELECTOR
        ).text
        if str(perihelionDistance) != str(
            asteroidFullData["orbital_data"]["perihelion_distance"]
        ):
            err.append(
                [
                    "asteroid in the index of  "
                    + str(counter)
                    + "  in the api array dose not have the same perihelion_distance in UI",
                    str(perihelionDistance),
                    str(asteroidFullData["orbital_data"]["perihelion_distance"]),
                ]
            )

        inclination = browser.find_element(By.XPATH, INCLINATION_SELECTOR).text
        if str(inclination) != str(asteroidFullData["orbital_data"]["inclination"]):
            err.append(
                [
                    "asteroid in the index of  "
                    + str(counter)
                    + "  in the api array dose not have the same inclination in UI",
                    str(inclination),
                    str(asteroidFullData["orbital_data"]["inclination"]),
                ]
            )
        ...
        if err != []:
            ...
            browser.save_screenshot(
                "./errors_screanshots/"
                + time.strftime("%-S_%M_%H_%d_%m_%Y")
                + " nasaAsteroidePage.png"
            )
        browser.quit()
        counter += 1
